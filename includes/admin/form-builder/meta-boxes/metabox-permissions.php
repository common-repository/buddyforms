...
function buddyforms_permissions_screen() {
    ...
    esc_html_e( 'Role', 'buddyforms' );
    ?></th>
				<th class="field_name">
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_1, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'Create', 'buddyforms' ) );
    ?>
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_2, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'Edit', 'buddyforms' ) );
    ?>
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_2, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'Delete', 'buddyforms' ) );
    ?>
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_2, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'Draft', 'buddyforms' ) );
    ?>
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_2, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'All Submissions', 'buddyforms' ) );
    ?>
					<?php 
    echo sprintf( '<span style="%s">%s</span>', wp_kses( $checkbox_style_group_2, buddyforms_wp_kses_allowed_atts() ), esc_html__( 'Admin Submission', 'buddyforms' ) );
    ?>
					<?php 
    if ( buddyforms_core_fs()->is_paying_or_trial() ) {
        ?>
    ...
    if ( isset( $form_setup ) ) {
        ...
        foreach ( $form_setup as $key => $field ) {
            ...
            if ( $type == 'html' ) {
                ...
                echo esc_attr( $class );
                ?>" id="table_row_<?php 
                echo esc_attr( $field->getAttribute( 'id' ) );
                ?>_<?php 
                echo esc_attr( $key );
                ?>" class=" <?php 
...
function buddyforms_form_setup_nav_li_permission() {
    ...
    esc_html_e( 'Permission', 'buddyforms' );
    ?></a></li>
																									<?php 
}
