...
function buddyforms_form_builder_templates(  $is_wizard = false  ) {
	...
	<div class="buddyforms_template buddyforms_template_container buddyforms_wizard_types">
		<div id="buddyforms_template_header_container">
			<div id="buddyforms_template_header_container_h3">
				<h3><?php 
    esc_html_e( 'Start adding Fields to your Form.', 'buddyforms' );
    ?></h3>
			</div>
			<div id="buddyforms_template_arrow_container">
				<img class="buddyforms_template_arrow" src="<?php 
    echo esc_attr( BUDDYFORMS_ASSETS ) . 'images/arrow.png';
    ?>">
			</div>
		</div>
    ...
    ?>
		...
		<div id="buddyforms_template_list_container">
			<h5><?php 
    esc_html_e( 'Choose a Form Template or build a custom Form. If you don\' find the template you need, drop us one <a href="mailto:support@themekraft.com">email to support@themekraft.com</a>.', 'buddyforms' );
    ?></h5>

			<div class="bf-3-tile bf-tile">
				<h4 class="bf-tile-title"><?php 
    esc_html_e( 'CUSTOM FORM', 'buddyforms' );
    ?></h4>
				<div class="xbf-col-50 bf-tile-desc-wrap">
					<p class="bf-tile-desc"><?php 
    esc_html_e( 'Select the field you want to use to build your form.', 'buddyforms' );
    ?></p>
				</div>
				...
				<div id="template-custom">
					...
					<button id="btn-compile-custom" data-type="" class="bf_form_template_custom button button-primary" onclick="">
						<?php 
    esc_html_e( 'Start Custom', 'buddyforms' );
    ?>
					</button>
				</div>
			</div>
    ...
    foreach ( $buddyforms_templates as $sort_key => $sort_item ) {
        ?>
				...
				<h2><?php 
        echo wp_kses( strtoupper( $sort_key ), buddyforms_wp_kses_allowed_atts() );
        ?>&nbsp;<?php 
        esc_html_e( 'FORMS', 'buddyforms' );
        ?></h2>
        ...
        foreach ( $sort_item as $key => $template ) {
            ...
            ?>
					">
						<h4 class="bf-tile-title"><?php 
            echo wp_kses_post( $template['title'] );
            ?></h4>
						<div class="xbf-col-50 bf-tile-desc-wrap">
							<p class="bf-tile-desc"><?php 
            echo wp_kses_post( wp_trim_words( $template['desc'], 15 ) );
            ?></p>
						</div>
						<div class="bf-tile-preview-wrap"></div>
						<?php 
            if ( $dependencies != $none_dependency_string ) {
                ...
                esc_html_e( 'Dependencies: ', 'buddyforms' );
                echo wp_kses( $dependencies, buddyforms_wp_kses_allowed_atts() );
                ?></p>
						<?php 
            } else {
                ?>
                ...
                ?>"
															...
															onclick="">
								<?php 
                esc_html_e( 'Use This Template', 'buddyforms' );
                ?>
							</button>
						<?php 
            }
            ...
            ?>" style="display:none;">
							<div class="bf-tile-desc-wrap">
								<p class="bf-tile-desc"><?php 
            echo wp_kses( $template['desc'], buddyforms_wp_kses_allowed_atts() );
            ?></p>
								<button <?php 
            echo esc_attr( $disabled );
            ?> id="btn-compile-<?php 
